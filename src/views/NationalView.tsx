import { useMemo, useState } from "react";
import type { AggregateMetric, CampaignListItem, DashboardDataset } from "../domain/types";
import { DataTable, Delta, KpiCard, KpiRow, PageHeader, SectionCard, Segmented } from "../ui";
import { IconBusinessUnit, IconNational, IconTeam } from "../ui/icons";
import { formatNumber, formatPercent, formatSignedPercent, formatWonThousand } from "../view-models/format";
import type { ComparisonIndex } from "../view-models/dashboard";
import { previousDeltaFor, previousTotalFor } from "../view-models/dashboard";
import { CampaignMeta, aggregateColumns } from "./shared";

type RankMetric = "daily" | "previous" | "focus";

const rankMetrics: { value: RankMetric; label: string }[] = [
  { value: "daily", label: "행사 일매출" },
  { value: "previous", label: "직전전단 대비" },
  { value: "focus", label: "중점 취급률" },
];

function rankValue(row: AggregateMetric, metric: RankMetric, index: ComparisonIndex) {
  if (metric === "daily") return row.eventDailySales;
  if (metric === "focus") return row.focusHandlingRate;
  return previousDeltaFor(index, row);
}

function rankText(value: number | undefined, metric: RankMetric) {
  if (value == null) return "-";
  if (metric === "daily") return `${formatWonThousand(value, 1)}천원`;
  if (metric === "focus") return formatPercent(value);
  return formatSignedPercent(value);
}

/**
 * Ranking strip. Bars are normalised to the largest absolute value in the list,
 * so the eye can order teams without reading every number first.
 */
function RankList({
  rows,
  metric,
  index,
  onSelect,
}: {
  rows: AggregateMetric[];
  metric: RankMetric;
  index: ComparisonIndex;
  onSelect?: (row: AggregateMetric) => void;
}) {
  const ranked = useMemo(() => {
    const withValue = rows
      .map((row) => ({ row, value: rankValue(row, metric, index) }))
      .filter((item) => item.value != null) as { row: AggregateMetric; value: number }[];
    return withValue.sort((a, b) => b.value - a.value);
  }, [rows, metric, index]);

  const scale = Math.max(...ranked.map((item) => Math.abs(item.value)), 0.0001);

  if (!ranked.length) return <p className="muted">비교할 데이터가 없습니다.</p>;

  return (
    <ol className="stack" data-gap="sm" style={{ listStyle: "none", margin: 0, padding: 0 }}>
      {ranked.map((item, position) => (
        <li key={item.row.key}>
          <button
            type="button"
            onClick={onSelect ? () => onSelect(item.row) : undefined}
            disabled={!onSelect}
            style={{
              display: "grid",
              gridTemplateColumns: "18px minmax(0, 1fr) 92px",
              alignItems: "center",
              gap: "var(--s-2)",
              width: "100%",
              border: 0,
              background: "transparent",
              padding: "2px 0",
              font: "inherit",
              textAlign: "left",
              cursor: onSelect ? "pointer" : "default",
            }}
          >
            <span className="muted num">{position + 1}</span>
            <span style={{ display: "grid", gap: 3, minWidth: 0 }}>
              <span className="trunc" style={{ ["--trunc-w" as string]: "100%" }} title={item.row.label}>
                {item.row.label}
              </span>
              <span className="meter-track">
                <span
                  className="meter-fill"
                  data-tone={item.value < 0 ? "danger" : undefined}
                  style={{ width: `${(Math.abs(item.value) / scale) * 100}%` }}
                />
              </span>
            </span>
            <span className="num" style={{ textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
              {metric === "previous" ? <Delta value={item.value} /> : rankText(item.value, metric)}
            </span>
          </button>
        </li>
      ))}
    </ol>
  );
}

export function NationalView({
  dataset,
  campaign,
  index,
  onOpenTeam,
}: {
  dataset: DashboardDataset;
  campaign?: CampaignListItem;
  index: ComparisonIndex;
  onOpenTeam?: (team: string) => void;
}) {
  const [rankMetric, setRankMetric] = useState<RankMetric>("daily");
  const national = dataset.aggregates.national;
  const canDrill = dataset.permissions.canViewAllStores;
  const nationalDelta = previousDeltaFor(index, national);
  const nationalPrevious = previousTotalFor(index, national);

  return (
    <div className="page">
      <PageHeader
        icon={<IconNational size={20} aria-hidden />}
        title="전국 현황"
        description={
          canDrill
            ? "전사 전단행사 실적 요약입니다. 부문·팀 행을 선택하면 하위 현황으로 이동합니다."
            : "전사 요약 지표입니다. 담당 점포 상세는 OFC 현황에서 확인하세요."
        }
        meta={<CampaignMeta dataset={dataset} campaign={campaign} />}
      />

      <KpiRow cols={6}>
        <KpiCard
          label="행사 총매출"
          value={formatWonThousand(national.eventSalesTotal)}
          unit="천원"
          emphasis
          note={`대상 ${formatNumber(national.storeCount)}점`}
        />
        <KpiCard
          label="행사 일매출"
          value={formatWonThousand(national.eventDailySales, 1)}
          unit="천원"
          emphasis
          hint="행사 총매출 ÷ 영업일수 합계"
          note={`영업일수 ${formatNumber(national.operatingDays)}일`}
        />
        <KpiCard
          label="직전전단 대비"
          value={formatSignedPercent(nationalDelta)}
          hint="행사 총매출을 직전전단 총매출과 비교한 증감률입니다."
          valueChange={nationalDelta}
          note={
            nationalPrevious != null
              ? `직전 ${formatWonThousand(nationalPrevious)}천원`
              : canDrill
                ? "직전 자료 없음"
                : "담당 범위 밖 (조회 권한)"
          }
        />
        <KpiCard
          label="신선 전체 일매출"
          value={formatWonThousand(national.freshDailySales, 1)}
          unit="천원"
          hint="신선매출 자료 기준 일매출입니다."
        />
        <KpiCard
          label="행사 신선 구성비"
          value={formatPercent(national.eventFreshComposition)}
          hint="신선 행사 일매출 ÷ 신선 전체 일매출"
        />
        <KpiCard
          label="중점상품 취급률"
          value={formatPercent(national.focusHandlingRate)}
          hint="Focus Unit 기준 취급 점포 비율입니다."
          note={`달성 ${formatNumber(national.focusAchievedStores)} / ${formatNumber(national.storeCount)}점`}
        />
      </KpiRow>

      {dataset.aggregates.businessUnits.length > 0 && (
        <SectionCard
          title="부문별 비교"
          icon={<IconBusinessUnit size={15} aria-hidden />}
          subtitle={`${formatNumber(dataset.aggregates.businessUnits.length)}개 부문`}
          flush
        >
          <DataTable
            rows={dataset.aggregates.businessUnits}
            rowKey={(row) => row.key}
            columns={aggregateColumns({ firstHeader: "부문", index, showOfcCount: true })}
            density="compact"
            autoHeight
            defaultSort={{ key: "sales", direction: "desc" }}
            caption="부문별 전단행사 실적 비교"
          />
        </SectionCard>
      )}

      {dataset.aggregates.teams.length > 0 && (
        <div className={dataset.aggregates.teams.length > 1 ? "grid-2" : "stack"} data-ratio="wide-left">
          <SectionCard
            title={canDrill ? "팀별 비교" : "소속팀 요약"}
            icon={<IconTeam size={15} aria-hidden />}
            subtitle={canDrill ? `${formatNumber(dataset.aggregates.teams.length)}개 팀 · 행 선택 시 팀 현황` : undefined}
            flush
          >
            <DataTable
              rows={dataset.aggregates.teams}
              rowKey={(row) => row.key}
              columns={aggregateColumns({ firstHeader: "영업팀", index, showOfcCount: true, variant: "slim" })}
              density="compact"
              autoHeight
              defaultSort={{ key: "sales", direction: "desc" }}
              onRowClick={canDrill && onOpenTeam ? (row) => onOpenTeam(row.label) : undefined}
              caption="팀별 전단행사 실적 비교"
            />
          </SectionCard>

          {dataset.aggregates.teams.length > 1 && (
          <SectionCard
            title="팀 순위"
            actions={
              <Segmented
                label="순위 기준 지표"
                value={rankMetric}
                options={rankMetrics}
                onChange={setRankMetric}
              />
            }
          >
            <RankList
              rows={dataset.aggregates.teams}
              metric={rankMetric}
              index={index}
              onSelect={canDrill && onOpenTeam ? (row) => onOpenTeam(row.label) : undefined}
            />
          </SectionCard>
          )}
        </div>
      )}
    </div>
  );
}
